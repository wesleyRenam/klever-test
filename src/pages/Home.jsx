...
function Home() {
  ...
  <div className='flex items-center m-3' key={eachToken.token}>
    <Link className='w-6 mr-1' to={`/edittoken/${eachToken.token}`}>
      <img src={edit} data-testid="editButton" alt="Botão de editar"/>
    </Link>
    <h1 data-testid="h1Token" className="text-white font-semibold text-3xl">{eachToken.token}</h1>
  </div> )

  // Lista de balances no LocalStorage
  const htmlBalance = tokens.map((eachToken) => 
  <div className="m-3 mr-0" key={eachToken.token}>
    <h1 data-testid="h1Balance"  className="text-white font-bold text-3xl flex justify-end">
  ...
  return (
    <div className='bg-backG h-screen w-screen items-center flex flex-col'>
      <div className='justify-center flex w-52 mb-16 mt-10'>
          <img src={logo} alt="Logo Klever" data-testid="logoKlever"/>
      </div>
      <div className='flex w-1/3  rounded-xl flex-col '>
        <div className='flex items-center justify-between'>
          <div className='flex'>
            <img src={star} alt="Estrela" className='w-12' data-testid='estrelaLogo'/>
            <h1 className='text-white font-bold text-5xl' data-testid='wishH1'>Wish Wallet</h1>
          </div>
          <Link data-testid="buttonAdd" name="buttonAdd" to='/addtoken' className='font-bold text-white bg-purple p-2 px-5 rounded-lg text-xl'>Add Token</Link>
        </div>
        {/* Verificando se tem tokens no localStorage para poder renderizar  */}
        { tokens.length === 0 ? <h1 data-testid="semToken"className='text-white font-medium text-3xl mt-20'> Você não tem nenhum token adicionado ainda</h1> :
